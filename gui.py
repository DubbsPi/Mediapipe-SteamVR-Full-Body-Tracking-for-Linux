...
class RealTimeGUI:
    def __init__(self):
        ...
        if self.data is None:
            # Default data
            self.data = {"camera": 0, "sliders": [0.2, 0.5, 0.15, 5.0, 25.0]}

        self.slider_data = self.data['sliders']

        # Variables that will be updated in real-time
        self.input_value = tk.IntVar(value=self.data['camera'])
        ...
        self.slider_configs = [
            ...
            {"name": "Brightness", "min": 0, "max": 50, "default": self.slider_data[4]}
        ]
    ...
    def setup_ui(self):
        ...
        input_entry = ttk.Entry(main_frame, textvariable=self.input_value, width=15)
        input_entry.grid(row=0, column=1, sticky=tk.W, pady=(0, 10))
        ...
        for i, config in enumerate(self.slider_configs):
            ...
            slider.grid(row=i+1, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=2)
            self.slider_widgets.append(slider)

            # Value label
            value_label = ttk.Label(main_frame, text=f"{config['default']:.1f}")
            value_label.grid(row=i+1, column=2, sticky=tk.W, padx=(10, 0), pady=2)
        ...
        toggle_btn.grid(row=len(self.slider_configs)+1, column=0, columnspan=2,
                       sticky=tk.W, pady=(20, 0))

        save_btn = tk.Button(main_frame, text="Save Config", command=self.save_config)
        save_btn.grid(row=len(self.slider_configs)+2, column=0, columnspan=2,
                       sticky=tk.W, pady=(20, 0))
